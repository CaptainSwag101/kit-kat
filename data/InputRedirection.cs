    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            if (gameTime.TotalGameTime.TotalSeconds != seconds)
            {
                ...
                kitkat.Program.ntrClient.sendHeartbeatPacket();
            }
        ...
        {
            kitkat.Program.scriptHelper.disconnect();
        }
        ...
        {
            if (!kitkat.Program.ntrClient.isConnected)
            {
                kitkat.Program.ntrClient.setServer(IPAddress, 8000);
                kitkat.Program.ntrClient.connectToServer(false);
            }
        }
        ...
        private void SendInput()
        {
            if ((newbuttons != oldbuttons) || (newtouch != oldtouch) || (newcpad != oldcpad))
            {
                ...
                if (kitkat.Program.ntrClient.isConnected)
                {
                    kitkat.Program.scriptHelper.write(0x10DF20, data, 0x10);
                }
            }
        }
    }
